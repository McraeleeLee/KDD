...
def evaluate(stdout, submit_fname,
    ...
    for i in range(1, len(schedule_in_unix_time) - 1):
        # 604800 == one week
        assert schedule_in_unix_time[i] + \
            604800 == schedule_in_unix_time[i + 1]
    ...
    try:
        ...
        return ValueError('server-side error: answer file incorrect')
    ...
    for phase_id in range(phase_beg, phase_end):
        ...
        try:
            ...
            return ValueError('error occurred during evaluation')
...
def _create_answer_file_for_evaluation_qtime(answer_fname='debias_track_answer.csv'):
    train = 'underexpose_train_click-%d.csv'
    test = 'underexpose_test_click-%d.csv'

    # underexpose_test_qtime-T.csv contains only <user_id, time>
    # underexpose_test_qtime_with_answer-T.csv contains <user_id, item_id, time>
    answer = 'underexpose_test_qtime-%d.csv'  # not released
